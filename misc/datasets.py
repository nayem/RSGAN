...
class Dataset(object):
    ...
    def transform(self, images):
        if self._aug_flag:
            transformed_images =\
                np.zeros([images.shape[0], images.shape[1], self._imsize, self._imsize, 3])
            ...
            for i in range(images.shape[0]):
                ...
                for j in range(images.shape[1]):
                    ...
                    h1 = int(np.floor((ori_size - self._imsize) * r1))
                    w1 = int(np.floor((ori_size - self._imsize) * r2))
                    #print("w1 = " + str(w1)+" done.")
                    #print("h1 = " + str(h1))
                    #print("self._imsize = " + str(self._imsize))
                    #print("i = " +str(i))
                    #print(images.shape)
                    cropped_image =\
                        images[i,j][w1: w1 + self._imsize, h1: h1 + self._imsize, :]
                    #print(cropped_image.shape)
                    if r3 > 0.5:
                        transformed_images[i,j] = np.fliplr(cropped_image)
                    else:
                        transformed_images[i,j] = cropped_image
...
class TextDataset(object):
    def __init__(self, workdir, embedding_type, hr_lr_ratio):
        ...
        elif embedding_type == 'fmri':
            self.embedding_filename = '/mri_embeddings.pickle'    
        elif embedding_type == 'glove':
            self.embedding_filename = '/word_embeddings.pickle'                       
...
class fMRIDataset(object):
    def __init__(self, workdir, embedding_type, hr_lr_ratio):
        ...
        if embedding_type == 'cnn-rnn':
            ...
            self.embedding_filename = '/mri.pickle'    
