...
@pt.Register
class custom_conv_lstm(pt.VarStoreMethod):
    def __call__(self, input_layer, output_dim, hidden,
                 k_h=5, k_w=5, d_h=2, d_w=2, stddev=0.02, in_dim=None, padding='SAME',
                 name="conv_lstm"):
        with tf.variable_scope(name, initializer = tf.random_uniform_initializer(-.01, 0.1)):
            cell = BasicConvLSTMCell.BasicConvLSTMCell([input_layer.shape[-3],input_layer.shape[-2]], [k_h,k_w,d_h,d_w], output_dim)
            if hidden is None:
