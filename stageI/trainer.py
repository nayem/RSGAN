...
class CondGANTrainer(object):
    ...
    def build_placeholder(self):
        ...
        self.images = tf.placeholder(
            tf.float32, [self.batch_size, cfg.SEQ_SIZE] + self.dataset.image_shape,
            name='real_images')
    ...
    def init_opt(self):
        ...
        with pt.defaults_scope(phase=pt.Phase.train):
            with tf.variable_scope("g_net"):
                ...
                fake_images, g_hidden = self.model.get_generator(tf.concat(axis=1, values=[c, z]), g_hidden)

            # ####get discriminator_loss and generator_loss ###################
            real_logit, d_real_hidden = self.model.get_discriminator(self.images[:,0,:,:], self.embeddings, d_real_hidden)
            wrong_logit, d_wrong_hidden = self.model.get_discriminator(self.wrong_images[:,0,:,:], self.embeddings, d_wrong_hidden)
            fake_logit, d_fake_hidden = self.model.get_discriminator(fake_images, self.embeddings, d_fake_hidden)
            discriminator_loss, generator_loss =\
                self.compute_losses(real_logit,
                                    wrong_logit,
                                    fake_logit)
            ...
            for i in range(1,cfg.SEQ_SIZE):
                print("creating model at time "+str(i))
                ...
                # ####get discriminator_loss and generator_loss ###################
                real_logit, d_real_hidden = self.model.get_discriminator(self.images[:,i,:,:], self.embeddings, d_real_hidden)
                wrong_logit, d_wrong_hidden = self.model.get_discriminator(self.wrong_images[:,i,:,:], self.embeddings, d_wrong_hidden)
                fake_logit, d_fake_hidden = self.model.get_discriminator(fake_images, self.embeddings, d_fake_hidden)
                discriminator_loss, generator_loss =\
                    self.compute_losses(real_logit,
                                        wrong_logit,
            ...
            self.prepare_trainer(generator_losses, discriminator_losses)
    ...
    def sampler(self):
        ...
        if cfg.TRAIN.FLAG:
            ...
            z = tf.random_normal([self.batch_size, cfg.Z_DIM])
        self.fake_images, g_test_hidden = self.model.get_generator(tf.concat(axis=1, values=[c, z]), g_test_hidden)

    def compute_losses(self, real_logit, wrong_logit, fake_logit):
    ...
    def prepare_trainer(self, generator_losses, discriminator_losses):
        '''Helper function for init_opt'''
    ...
    def epoch_sum_images(self, sess, n):
        ...
        for row in range(n):
            ...
            pfi_test.write('\n***row %d***\n' % row)
            if not captions_test:
                pfi_test.write(str(row * n))
            else:
