...
class CondGAN(object):
    def __init__(self, image_shape):
        ...
        if cfg.GAN.NETWORK_TYPE == "default":
            with tf.variable_scope("d_net"):
                ...
                self.discriminator_2nd_template = self.discriminator_2nd()                
    ...
    def generator(self, z_var, g_hidden):
        node1_0 =\
            (pt.wrap(z_var).
             flatten().
        ...
        node1, hidden = \
            (node1_0.
             ...
             apply(tf.nn.relu).
             custom_conv_lstm(self.gf_dim * 8, g_hidden, k_h=3, k_w=3, d_h=1, d_w=1)
             )
    ...
    def get_generator(self, z_var, g_hidden):
        if cfg.GAN.NETWORK_TYPE == "default":
            return self.generator(z_var, g_hidden)
        elif cfg.GAN.NETWORK_TYPE == "simple":
    ...
    def get_discriminator(self, x_var, c_var, d_hidden):
        ...
        node1, node2 = self.discriminator_template.construct(input=x_c_code, hidden=d_hidden)
        node3 = self.discriminator_2nd_template.construct(input = node1)
        return node3, node2 
